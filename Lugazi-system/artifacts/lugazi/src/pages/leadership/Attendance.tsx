...
export default function LeadershipAttendance() {
  ...
  function handleQrScan() {
    qrScanMutation.mutate({ data: { qrToken, eventName } }, {
      onSuccess: d => { queryClient.invalidateQueries({ queryKey: getListAttendanceQueryKey() }); toast({ title: `Checked in: ${d.memberName}` }); setShowQr(false); },
      onError: () => toast({ title: "QR token not found", variant: "destructive" }),
    });
  }

  function handleManual() {
    checkInMutation.mutate({ data: { memberId: Number(memberId), eventName, method: "manual" } }, {
      onSuccess: d => { queryClient.invalidateQueries({ queryKey: getListAttendanceQueryKey() }); toast({ title: `Checked in: ${d.memberName}` }); setShowManual(false); },
    });
  }
  ...
  return (
    <PortalLayout navItems={leadershipNavItems} portalLabel="Leadership Portal">
      ...
      <DataTable
        columns={[
          { header: "Member", key: "memberName" }, { header: "Event", key: "eventName" },
          { header: "Method", key: "method" }, { header: "Time", key: "checkedInAt", render: r => new Date(r.checkedInAt).toLocaleString() },
        ]}
      ...
      <Dialog open={showQr} onOpenChange={setShowQr}>
        <DialogContent>
          ...
          <div className="space-y-3">
            <div><Label>QR Token</Label><Input value={qrToken} onChange={e => setQrToken(e.target.value)} placeholder="Paste QR token" data-testid="input-qr-token" /></div>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleQrScan} disabled={qrScanMutation.isPending} data-testid="button-confirm-qr">Check In</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      <Dialog open={showManual} onOpenChange={setShowManual}>
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleManual} disabled={checkInMutation.isPending} data-testid="button-confirm-manual">Check In</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </PortalLayout>
