...
type PipelineContact = { id: number; name: string; phone: string; stage: string; source: string };
...
export default function LeadershipPipeline() {
  ...
  return (
    <PortalLayout navItems={leadershipNavItems} portalLabel="Leadership Portal">
      <PageHeader title="Soul-Winning Pipeline" actions={<Button size="sm" onClick={() => { setForm(blank); setShowAdd(true); }} data-testid="button-add-contact"><Plus className="h-4 w-4 mr-1" /> Add</Button>} />
      <DataTable
        columns={[
          { header: "Name", key: "name" }, { header: "Phone", key: "phone" }, { header: "Source", key: "source" },
          ...
          { header: "Actions", key: "actions", render: r => <button className="text-muted-foreground hover:text-destructive" onClick={() => deleteMutation.mutate({ id: r.id }, { onSuccess: () => queryClient.invalidateQueries({ queryKey: getListPipelineQueryKey() }) })} data-testid={`button-delete-${r.id}`}><Trash2 className="h-4 w-4" /></button> },
        ]}
        data={contacts as PipelineContact[]} keyField="id" isLoading={isLoading} emptyMessage="No contacts in pipeline."
      />
