...
export default function LeadershipSermons() {
  ...
  const SermonForm = () => (
    <div className="space-y-4 max-h-[65vh] overflow-y-auto pr-1">
      <div className="grid grid-cols-2 gap-3">
        ...
        <div><Label>Preacher *</Label><Input value={form.preacher} onChange={e => setForm(f => ({ ...f, preacher: e.target.value }))} className="mt-1" /></div>
      </div>
      <div className="grid grid-cols-2 gap-3">
        <div><Label>Date *</Label><Input type="date" value={form.sermonDate} onChange={e => setForm(f => ({ ...f, sermonDate: e.target.value }))} className="mt-1" /></div>
        <div><Label>Series</Label><Input value={form.series} onChange={e => setForm(f => ({ ...f, series: e.target.value }))} className="mt-1" /></div>
      </div>
      <div><Label>Scripture Reference</Label><Input value={form.scriptureRef} onChange={e => setForm(f => ({ ...f, scriptureRef: e.target.value }))} placeholder="e.g. John 3:16" className="mt-1" /></div>
      ...
      <div><Label className="mb-2 block">Upload Media</Label>
      ...
      <div><Label className="mb-2 block">Thumbnail (optional)</Label>
  ...
  return (
    <PortalLayout navItems={leadershipNavItems} portalLabel="Leadership Portal">
      <PageHeader title="Sermon Library" description={`${(sermons as Sermon[]).length} sermons`}
        actions={<Button size="sm" onClick={() => { resetForm(); setShowAdd(true); }} className="blue-gradient-bg text-white border-0"><Plus className="h-4 w-4 mr-1" /> Add Sermon</Button>} />

      {isLoading ? <div className="grid grid-cols-1 md:grid-cols-2 gap-4">{Array.from({ length: 4 }).map((_, i) => <div key={i} className="h-28 rounded-xl bg-muted animate-pulse" />)}</div>
        : (sermons as Sermon[]).length === 0 ? (
          <div className="text-center py-16 text-muted-foreground"><Mic2 className="h-12 w-12 mx-auto mb-3 opacity-30" /><p>No sermons yet.</p></div>
        ) : (
