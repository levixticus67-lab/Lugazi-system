...
router.post("/ai/assist", requireAuth, async (req: AuthRequest, res): Promise<void> => {
  ...
  if (!apiKey) {
    res.json({
      response: "AI Assistant is not yet configured. To enable AI features, add your GEMINI_API_KEY to the Render environment variables in your Render dashboard under Environment → Environment Variables.",
      error: "no_api_key",
    });
    return;
  }
