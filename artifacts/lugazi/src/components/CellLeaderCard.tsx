...
function Avatar({ name, photoUrl }: { name: string; photoUrl?: string | null }) {
  if (photoUrl) return <img loading="lazy" src={cldAvatar(photoUrl)} alt={name} className="w-8 h-8 rounded-full object-cover shrink-0" />;
  return (
...
export default function CellLeaderCard() {
  ...
  if (isLoading) return <div className="glass-card p-4 animate-pulse h-28 rounded-xl" />;
      ...
      <div className="glass-card p-4 animate-slide-in-up border-l-4 border-sky-500">
        <div className="flex items-start justify-between gap-2 mb-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="text-[10px] text-sky-500 font-semibold uppercase tracking-wide">Your Cell Fellowship</p>
        ...
        <div className="flex flex-wrap gap-3 mb-3 text-[11px] text-muted-foreground">
          ...
          {group.meetingDay && (
            <span className="flex items-center gap-1">
              <Clock className="h-3 w-3" /> {group.meetingDay}{group.meetingTime ? ` · ${group.meetingTime}` : ""}
            </span>
          )}
        ...
        {(group.members ?? []).length === 0 ? (
          <p className="text-xs text-muted-foreground text-center py-2">No members yet. Add members to your cell fellowship.</p>
        ) : (
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="glass-card w-full max-w-md p-5 space-y-4 max-h-[80vh] flex flex-col">
            ...
            <div className="relative">
              ...
              <input
                autoFocus
                className="w-full bg-muted rounded-lg pl-9 pr-3 py-2 text-sm outline-none focus:ring-2 focus:ring-primary/30"
            ...
            <div className="flex-1 overflow-y-auto space-y-1">
              {filteredAll.length === 0 ? (
                <p className="text-xs text-muted-foreground text-center py-4">
                  {search ? "No matching members found" : "All members are already in this fellowship"}
                </p>
              ) : (
                filteredAll.map(m => (
                  <button
