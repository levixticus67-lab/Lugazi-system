  ...
  if (item.type === "image") {
    return (
      <div className="aspect-video bg-muted overflow-hidden relative cursor-pointer group" onClick={onClick}>
        <img loading="lazy" src={cldThumb(item.url)} alt={item.title} className="w-full h-full object-cover transition-transform group-hover:scale-105" onError={(e) => { (e.target as HTMLImageElement).style.display = "none"; }} />
