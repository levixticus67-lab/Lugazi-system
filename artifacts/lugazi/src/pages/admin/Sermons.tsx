...
export default function AdminSermons() {
  ...
  const activeTitle = editSermon ? "Edit Sermon" : "Add Sermon";
  ...
  return (
    <PortalLayout navItems={adminNavItems} portalLabel="Admin Portal">
      ...
      {/* Custom overlay modal — avoids Dialog focus-trap keyboard issue on mobile */}
      {activeForm && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
          <div className="bg-background w-full sm:max-w-lg rounded-t-2xl sm:rounded-2xl shadow-2xl flex flex-col max-h-[92vh]">
            <div className="flex items-center justify-between px-5 pt-5 pb-3 border-b border-border shrink-0">
              <h2 className="font-semibold text-base">{activeTitle}</h2>
            ...
            <div className="overflow-y-auto flex-1 px-5 py-4 space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    className="w-full bg-muted rounded-lg px-3 py-2 text-sm border-0 outline-none focus:ring-2 focus:ring-primary/30"
                    value={form.title}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    onChange={e => setForm(f => ({ ...f, sermonDate: e.target.value }))}
                  />
                </div>
                <div>
              ...
              <div>
                ...
                <textarea
                  className="w-full bg-muted rounded-lg px-3 py-2 text-sm border-0 outline-none focus:ring-2 focus:ring-primary/30 resize-none"
                  rows={2}
            ...
            <div className="flex gap-3 px-5 py-4 border-t border-border shrink-0">
              <button
                onClick={() => { setShowAdd(false); setEditSermon(null); resetForm(); }}
              ...
              >{saving ? "Saving…" : editSermon ? "Save Changes" : "Add Sermon"}</button>
            </div>
          </div>
        </div>
      )}
