  ...
  const SermonForm = () => (
    <div className="space-y-4 max-h-[65vh] overflow-y-auto pr-1">
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div><Label>Series</Label><Input value={form.series} onChange={e => setForm(f => ({ ...f, series: e.target.value }))} placeholder="e.g. Faith Series" className="mt-1" /></div>
      </div>
      ...
      <div>
        <Label className="mb-2 block">Upload Sermon Media (audio, video, or document)</Label>
        <CloudinaryUploader
          accept="audio/*,video/*,.pdf,.doc,.docx"
          label="Upload media file"
          onUpload={setMediaResult}
          currentUrl={mediaResult?.url ?? (editSermon?.mediaUrl ?? undefined)}
        />
      </div>
      <div>
        ...
        <CloudinaryUploader
          ...
          onUpload={setThumbResult}
          currentUrl={thumbResult?.url ?? (editSermon?.thumbnailUrl ?? undefined)}
        />
      </div>
    </div>
  );
              ...
              <div className="flex gap-3">
                {s.thumbnailUrl ? (
                  ...
                  <div className="w-16 h-16 rounded-lg blue-gradient-bg flex items-center justify-center flex-shrink-0">
                    {s.mediaType === "document" ? <FileText className="h-7 w-7 text-white" /> : <Mic2 className="h-7 w-7 text-white" />}
                  </div>
                )}
