...
export default function AdminWelfare() {
  ...
  function handleUpdate() {
    ...
    updateMutation.mutate({ id: reviewReq.id, data: { status, adminNote: adminNote || undefined } }, {
      onSuccess: () => {
        queryClient.invalidateQueries({ queryKey: getListWelfareQueryKey() });
  ...
  async function handleDelete(id: number) {
    if (!confirm("Delete this welfare request permanently?")) return;
    setDeleting(id);
    try {
      await axios.delete(`/api/welfare/${id}`);
      queryClient.invalidateQueries({ queryKey: getListWelfareQueryKey() });
    ...
    <PortalLayout navItems={adminNavItems} portalLabel="Admin Portal">
      ...
      {isLoading ? (
        ...
        <div className="glass-card p-12 text-center">
          ...
          <p className="text-muted-foreground font-medium">No welfare requests {filterStatus !== "all" ? `with status "${filterStatus}"` : "yet"}</p>
        </div>
      ) : (
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-2 flex-wrap">
                    ...
                    <div className="flex items-center gap-2 shrink-0 flex-wrap">
                      <span className={`flex items-center gap-1 text-[10px] px-2 py-0.5 rounded-full font-medium ${cfg.color}`}>
                        {cfg.icon}{cfg.label}
                      </span>
                      {r.amountRequested && (
                        <span className="text-xs font-medium text-foreground">UGX {Number(r.amountRequested).toLocaleString()}</span>
                      )}
                    </div>
                  </div>
                  <p className="text-xs text-muted-foreground mt-2 line-clamp-2">{r.description}</p>
                  {r.adminNote && (
                    <p className="text-xs mt-1.5 italic text-muted-foreground">Note: {r.adminNote}</p>
                  )}
                  <div className="flex items-center justify-between mt-3 flex-wrap gap-2">
                    ...
                    <div className="flex gap-2">
                      ...
                      <Button size="sm" variant="outline" className="h-7 text-xs gap-1 hover:bg-destructive/10 hover:text-destructive hover:border-destructive/30"
                        disabled={deleting === r.id} onClick={() => handleDelete(r.id)}>
                        <Trash2 className="h-3 w-3" />{deleting === r.id ? "…" : "Delete"}
                      </Button>
                    </div>
                  </div>
                </div>
          ...
          {reviewReq && (
            <div className="space-y-4 py-1">
              <div className="p-3 rounded-xl bg-muted/60 text-sm">
                ...
                <p className="text-xs text-muted-foreground mt-0.5 capitalize">{reviewReq.category.replace(/_/g," ")} · {reviewReq.description.slice(0,80)}{reviewReq.description.length>80?"…":""}</p>
              </div>
              <div>
                <Label>Status</Label>
