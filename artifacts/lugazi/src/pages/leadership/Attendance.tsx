...
export default function LeadershipAttendance() {
  ...
  function handleQrScan(token: string) {
    ...
    toast({ title: "QR code scanned", description: "Now select the event." });
  }

  function handleQrCheckIn() {
    if (!scannedToken || !selectedEventId) {
      toast({ title: "Scan a QR code and select an event first", variant: "destructive" }); return;
    }
    ...
    qrScanMutation.mutate({ data: { qrToken: scannedToken, eventName: event.title, eventId: Number(selectedEventId) } }, {
      onSuccess: d => { queryClient.invalidateQueries({ queryKey: getListAttendanceQueryKey() }); toast({ title: `Checked in: ${d.memberName}` }); setShowQr(false); setScannedToken(""); setSelectedEventId(""); },
      onError: () => toast({ title: "QR token not found", variant: "destructive" }),
    });
  }

  function handleManual() {
    if (!memberId || !selectedEventId) {
      toast({ title: "Select a member and event", variant: "destructive" }); return;
    }
    const event = getEventById(selectedEventId);
    if (!event) return;
    checkInMutation.mutate({ data: { memberId: Number(memberId), eventName: event.title, eventId: Number(selectedEventId), method: "manual" } }, {
      onSuccess: d => { queryClient.invalidateQueries({ queryKey: getListAttendanceQueryKey() }); toast({ title: `Checked in: ${d.memberName}` }); setShowManual(false); setMemberId(""); setSelectedEventId(""); },
  ...
  return (
    <PortalLayout navItems={leadershipNavItems} portalLabel="Leadership Portal">
      ...
      <DataTable
        ...
        keyField="id" isLoading={isLoading} emptyMessage="No attendance records yet."
      />
      ...
      <Dialog open={showQr} onOpenChange={setShowQr}>
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-4">
            ...
            {scannedToken && <p className="text-xs text-green-600 bg-green-50 border border-green-200 rounded-lg px-3 py-2">✓ QR Code scanned</p>}
            <div>
              <Label>Select Event</Label>
              <Select value={selectedEventId} onValueChange={setSelectedEventId}>
                <SelectTrigger className="mt-1"><SelectValue placeholder="Choose event…" /></SelectTrigger>
                <SelectContent>{eventOptions.map(e => <SelectItem key={e.id} value={String(e.id)}>{e.title} — {new Date(e.date).toLocaleDateString()}</SelectItem>)}</SelectContent>
              </Select>
            </div>
          </div>
          <DialogFooter>
      ...
      <Dialog open={showManual} onOpenChange={setShowManual}>
        <DialogContent className="max-w-md">
          ...
          <DialogFooter>
            ...
            <Button onClick={handleManual} disabled={checkInMutation.isPending}>Check In</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </PortalLayout>
