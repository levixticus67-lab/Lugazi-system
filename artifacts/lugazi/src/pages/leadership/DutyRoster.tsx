import { useState } from "react";
  ...
  export default function LeadershipDutyRoster() {
    ...
    const createMut = useMutation({
      ...
      onSuccess: () => { qc.invalidateQueries({queryKey:["leadership-duty-roster"]}); toast({title:"Duty scheduled"}); setShowCreate(false); setForm({assignedToUserId:"",assignedToName:"",serviceDate:"",serviceType:"Sunday Service",dutyRole:"Usher",location:"",notes:""}); },
    });

    const deleteMut = useMutation({
      mutationFn: (id: number) => axios.delete(`/api/duty-roster/${id}`),
      onSuccess: () => { qc.invalidateQueries({queryKey:["leadership-duty-roster"]}); toast({title:"Removed"}); },
    });
    ...
    return (
      <PortalLayout navItems={leadershipNavItems} portalLabel="Leadership Portal">
        <PageHeader title="Duty Roster" description="Schedule service duties for the workforce" actions={
          <Button size="sm" onClick={()=>setShowCreate(true)} className="blue-gradient-bg text-white border-0 hover:opacity-90"><Plus className="h-4 w-4 mr-1"/>Schedule</Button>
        } />

        {isLoading ? <div className="space-y-2">{[...Array(4)].map((_,i)=><div key={i} className="glass-card p-4 h-14 animate-pulse"/>)}</div>
         : duties.length === 0 ? (
        ...
        <Dialog open={showCreate} onOpenChange={setShowCreate}>
          <DialogContent>
            <DialogHeader><DialogTitle>Schedule Duty</DialogTitle></DialogHeader>
            <div className="space-y-3">
              <div><Label>Assign To *</Label>
                <Select value={form.assignedToUserId} onValueChange={handleUserSelect}>
                  <SelectTrigger className="mt-1"><SelectValue placeholder="Select workforce…"/></SelectTrigger>
