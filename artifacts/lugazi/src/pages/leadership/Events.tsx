...
export default function LeadershipEvents() {
  ...
  const blank = { title: "", description: "", date: "", time: "09:00", location: "", category: "service" };
  ...
  return (
    <PortalLayout navItems={leadershipNavItems} portalLabel="Leadership Portal">
      <PageHeader title="Events" description={`${upcoming.length} upcoming · ${past.length} past`}
        actions={<Button size="sm" onClick={() => { setForm(blank); setShowAdd(true); }}><Plus className="h-4 w-4 mr-1" />Add Event</Button>} />
      ...
      <Dialog open={showAdd} onOpenChange={setShowAdd}>
        <DialogContent className="max-w-md">
          ...
          <DialogFooter>
            ...
            <Button onClick={handleAdd} disabled={!form.title||!form.date||createMutation.isPending}>{createMutation.isPending?"Creating…":"Create Event"}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </PortalLayout>
