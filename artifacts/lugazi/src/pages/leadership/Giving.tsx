...
export default function LeadershipGiving() {
  const { user } = useAuth();
  ...
  const { data: contributions = [], isLoading } = useQuery<Contribution[]>({
    queryKey: ["my-contributions-leadership"],
  ...
  const create = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["my-contributions-leadership"] }); setShowForm(false); setForm({ type:"tithe", amount:"", currency:"UGX", reference:"", notes:"" }); },
  });
