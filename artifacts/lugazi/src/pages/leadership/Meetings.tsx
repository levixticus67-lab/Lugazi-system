...
export default function LeadershipMeetings() {
  ...
  // Attendance marking for a selected meeting
  ...
  // Toggle a name in/out of a list
  function toggle(list: string[], setList: (v: string[]) => void, name: string) {
    setList(list.includes(name) ? list.filter(n => n !== name) : [...list, name]);
  }

  const filteredForCreate = (members as Member[])
    .filter(m => m.fullName.toLowerCase().includes(attendeeSearch.toLowerCase()))
    .slice(0, 8);

  const filteredForMark = (members as Member[])
    .filter(m => m.fullName.toLowerCase().includes(markSearch.toLowerCase()))
    .slice(0, 10);

  function openMeeting(m: Meeting) {
    setSelected(m);
    const existing = m.attendees ? m.attendees.split(",").map(s=>s.trim()).filter(Boolean) : [];
    setMarkedAttendees(existing);
    setMarkSearch("");
    setEditingNotes(false);
                  ...
                  <div className="flex flex-wrap gap-1.5 mb-2">
                    {markedAttendees.map(n => (
                      <span key={n} className="flex items-center gap-1 text-[10px] px-2 py-1 rounded-full bg-green-100 text-green-700 dark:bg-green-950 dark:text-green-300 font-medium">
                        ...
                        {n}
                        <button type="button" onClick={() => toggle(markedAttendees, setMarkedAttendees, n)}
                ...
                <div className="flex flex-wrap gap-1">
                  {selectedAttendees.map(n => (
                    <span key={n} className="flex items-center gap-1 text-[10px] px-2 py-0.5 rounded-full bg-primary/10 text-primary">
                      {n}
                      <button type="button" onClick={() => toggle(selectedAttendees, setSelectedAttendees, n)}>
                        <X className="h-2.5 w-2.5" />
                      </button>
                    </span>
                  ))}
