    ...
    <PortalLayout navItems={leadershipNavItems} portalLabel="Leadership Portal">
        ...
        <div className="glass-card p-4 mb-4">
          ...
          <div className="flex h-2.5 rounded-full overflow-hidden gap-px">
            {STAGES.map(s => {
              ...
              return pct > 0 ? <div key={s} className={`${STAGE_CONFIG[s].dot} transition-all`} style={{ width: `${pct}%` }} title={STAGE_CONFIG[s].label} /> : null;
            })}
          </div>
          <div className="flex gap-3 mt-2 flex-wrap">
            {STAGES.map(s => (
              <span key={s} className="flex items-center gap-1 text-[10px] text-muted-foreground">
                <span className={`w-2 h-2 rounded-full ${STAGE_CONFIG[s].dot}`} />{STAGE_CONFIG[s].label}: {all.filter(c=>c.stage===s).length}
              </span>
            ))}
          </div>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="glass-card p-12 text-center">
          ...
          <p className="text-muted-foreground font-medium">No contacts {activeStage !== "all" ? `in "${STAGE_CONFIG[activeStage]?.label}"` : "yet"}</p>
        </div>
      ) : (
      ...
      <Dialog open={showAdd} onOpenChange={setShowAdd}>
        <DialogContent className="max-w-md">
          ...
          <DialogFooter>
            ...
            <Button onClick={handleAdd} disabled={!form.name||!form.phone||createMutation.isPending}>{createMutation.isPending?"Adding…":"Add Contact"}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </PortalLayout>
