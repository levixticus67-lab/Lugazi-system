import { useState } from "react";
  ...
  import { Plus, Trash2, ClipboardList, Clock, User } from "lucide-react";
  ...
  export default function LeadershipTasks() {
    ...
    const { data: users = [] } = useQuery<UserOption[]>({
      ...
      queryFn: () => axios.get<UserOption[]>("/api/users").then(r => r.data.filter(u => u.role === "workforce")),
    });

    const createMut = useMutation({
      mutationFn: (d: typeof form) => axios.post("/api/tasks", {...d, assignedToUserId: d.assignedToUserId ? Number(d.assignedToUserId) : undefined}),
      onSuccess: () => { qc.invalidateQueries({queryKey:["leadership-tasks"]}); toast({title:"Task assigned"}); setShowCreate(false); setForm({title:"",description:"",assignedToUserId:"",assignedToName:"",dueDate:"",priority:"medium",category:"",notes:""}); },
    });

    const deleteMut = useMutation({
      mutationFn: (id: number) => axios.delete(`/api/tasks/${id}`),
      onSuccess: () => { qc.invalidateQueries({queryKey:["leadership-tasks"]}); toast({title:"Task removed"}); },
    });

    const updateMut = useMutation({
      mutationFn: ({id,status}: {id:number;status:string}) => axios.patch(`/api/tasks/${id}`,{status}),
      onSuccess: () => qc.invalidateQueries({queryKey:["leadership-tasks"]}),
    });
    ...
    return (
      <PortalLayout navItems={leadershipNavItems} portalLabel="Leadership Portal">
        <PageHeader title="Task Assignments" description="Assign and track ministry tasks for your workforce" actions={
          <Button size="sm" onClick={()=>setShowCreate(true)} className="blue-gradient-bg text-white border-0 hover:opacity-90"><Plus className="h-4 w-4 mr-1"/>Assign Task</Button>
        } />

        {isLoading ? <div className="space-y-3">{[...Array(4)].map((_,i)=><div key={i} className="glass-card p-5 h-24 animate-pulse"/>)}</div>
         : tasks.length === 0 ? (
          <div className="glass-card p-12 text-center"><ClipboardList className="h-10 w-10 mx-auto text-muted-foreground/40 mb-3"/><p className="text-muted-foreground">No tasks yet. Assign your first task to the workforce.</p></div>
        ) : (
          <div className="space-y-3">
            {tasks.map(task => (
              <div key={task.id} className="glass-card p-5 flex items-start gap-3">
                <div className="flex-1">
                  ...
                  <div className="flex flex-wrap gap-4 text-xs text-muted-foreground mt-1">
                    {task.assignedToName && <span className="flex items-center gap-1"><User className="h-3 w-3"/>To: <span className="text-foreground font-medium">{task.assignedToName}</span></span>}
                ...
                <div className="flex gap-2 shrink-0">
                  ...
                  <Button size="sm" variant="ghost" className="text-destructive h-7 w-7 p-0" onClick={()=>deleteMut.mutate(task.id)}><Trash2 className="h-3.5 w-3.5"/></Button>
                </div>
              </div>
            ))}
          </div>
        ...
        <Dialog open={showCreate} onOpenChange={setShowCreate}>
          <DialogContent className="max-w-md">
            <DialogHeader><DialogTitle>Assign Task to Workforce</DialogTitle></DialogHeader>
            <div className="space-y-3">
              ...
              <div><Label>Assign To *</Label>
                <Select value={form.assignedToUserId} onValueChange={handleUserSelect}>
                  <SelectTrigger className="mt-1"><SelectValue placeholder="Select workforce member…"/></SelectTrigger>
                  <SelectContent>{users.map(u=><SelectItem key={u.id} value={String(u.id)}>{u.displayName}</SelectItem>)}</SelectContent>
                </Select>
              </div>
              ...
              <div><Label>Category</Label><Input value={form.category} onChange={e=>setForm(f=>({...f,category:e.target.value}))} className="mt-1" placeholder="e.g. Worship, Outreach"/></div>
            ...
            <DialogFooter>
              ...
              <Button onClick={()=>createMut.mutate(form)} disabled={!form.title||!form.assignedToUserId||createMut.isPending} className="blue-gradient-bg text-white border-0 hover:opacity-90">Assign Task</Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
      </PortalLayout>
