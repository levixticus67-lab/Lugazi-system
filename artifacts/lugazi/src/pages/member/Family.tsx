...
interface LinkedToMe extends FamilyMember {
...
export default function MemberFamily() {
  ...
  function selectChurchMember(cm: ChurchMember) {
    setForm(p => ({
      ...
      linkedMemberId: cm.id,
      linkedUserId: cm.userId ?? null,
    }));
  ...
  return (
    <PortalLayout title="DCL Lugazi ERP" navItems={memberNavItems}>
      ...
      <div className="p-6 space-y-8">
        ...
        {/* ── People who listed me as family ───────────────── */}
        {linkedToMe.length > 0 && (
          <section>
            <h2 className="text-sm font-semibold text-muted-foreground uppercase tracking-wide mb-3 flex items-center gap-2">
              <Heart className="h-4 w-4 text-rose-500"/> Listed as my family by others
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {linkedToMe.map(m=>(
                <div key={m.id} className="glass-card p-5 border-l-4 border-rose-400">
                  <div className="flex items-start gap-3">
                    <div className="w-10 h-10 rounded-full bg-rose-100 flex items-center justify-center text-rose-600 font-bold text-sm shrink-0">
                      {(m.addedByName ?? m.fullName).charAt(0)}
                    </div>
                    <div className="flex-1">
                      <div className="flex items-center gap-2 flex-wrap">
                        <span className="font-semibold">{m.addedByName ?? "Church member"}</span>
                        <span className={`text-[10px] px-2 py-0.5 rounded-full font-medium ${relColors[m.relationship]??"bg-slate-100 text-slate-600"}`}>
                          their {m.relationship}
                        </span>
                      </div>
                      <p className="text-xs text-muted-foreground mt-1">
                        Added you as their {m.relationship.toLowerCase()} on {new Date(m.createdAt).toLocaleDateString("en-GB",{day:"numeric",month:"short",year:"numeric"})}
                      </p>
                    </div>
                  </div>
                </div>
          ...
          <div className="glass-card w-full max-w-md p-6 space-y-4 max-h-[90vh] overflow-y-auto">
            ...
            {!editing && (
              <div>
                      ...
                      {filteredMembers.map(cm=>(
                        <button key={cm.id} onMouseDown={()=>selectChurchMember(cm)}
                          ...
                          <div className="w-8 h-8 rounded-full blue-gradient-bg flex items-center justify-center text-white text-xs font-bold shrink-0">
                            {cm.photoUrl ? <img src={cm.photoUrl} className="w-8 h-8 rounded-full object-cover"/> : cm.fullName.charAt(0)}
                          </div>
                ...
                {form.linkedUserId && (
                  <p className="text-xs text-green-600 mt-1 flex items-center gap-1"><UserCheck className="h-3 w-3"/>Linked to a church member — they will be notified.</p>
                )}
            ...
            {[
              ...
              <div key={f.key}>
                ...
                <input type={f.type} className="w-full bg-muted rounded-lg px-3 py-2 text-sm" placeholder={f.placeholder} value={form[f.key as keyof typeof blankForm] as string} onChange={e=>setForm(p=>({...p,[f.key]:e.target.value}))} />
              </div>
            ))}
