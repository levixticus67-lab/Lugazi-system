...
export default function WorkforceGiving() {
  const { user } = useAuth();
  ...
  const { data: contributions = [], isLoading } = useQuery<Contribution[]>({
    queryKey: ["my-contributions-workforce"],
  ...
  const create = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["my-contributions-workforce"] }); setShowForm(false); setForm({ type:"tithe", amount:"", currency:"UGX", reference:"", notes:"" }); },
  });
