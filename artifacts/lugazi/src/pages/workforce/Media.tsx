        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {(items as MediaItem[]).map(item => (
            <div key={item.id} className="bg-card border border-card-border rounded-xl overflow-hidden shadow-sm" data-testid={`media-${item.id}`}>
              {item.type === "image" && <div className="aspect-square bg-muted overflow-hidden"><img loading="lazy" src={cldThumb(item.url)} alt={item.title} className="w-full h-full object-cover" onError={e => { (e.target as HTMLImageElement).style.display = "none"; }} /></div>}
